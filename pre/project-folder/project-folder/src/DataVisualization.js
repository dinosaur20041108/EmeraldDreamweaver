...
function DataVisualization({ parameters }) {
    ...
    datasets: [
      {
        ...
        data: Electricicy_data.slice(currentStartYear - 1990, currentStartYear - 1980).map(value => value / parameters.energyGrowth),
        borderColor: "blue",
        fill: false,
      },
    ]
  ...
  const currentYearData = {
    Mt_CO2e: Mt_CO2e_data[currentStartYear - 1990] * parameters.gasEmission * (0.9 + 0.1 * parameters.carToElectricRatio) * parameters.energyGrowth,
    humman: humman_data[currentStartYear - 1990] * parameters.gasEmission * (0.9 + 0.1 * parameters.carToElectricRatio) * parameters.energyGrowth,
    Electricicy: Electricicy_data[currentStartYear - 1990] * parameters.gasEmission * (0.9 + 0.1 * parameters.carToElectricRatio) * parameters.energyGrowth,
  };
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: "24px", color: "#fff" }}>Year ({currentStartYear})</h3>
        <p style={{ fontSize: "16px", margin: "5px 0", color: "#fff" }}><strong>CO2 Emissions:</strong> {currentYearData.Mt_CO2e.toExponential(2)} Mt</p>
        <p style={{ fontSize: "16px", margin: "5px 0", color: "#fff" }}><strong>Population:</strong> {(currentYearData.humman / 1000000).toExponential(2)} million</p>
        <p style={{ fontSize: "16px", margin: "5px 0", color: "#fff" }}><strong>Electricity Consumption:</strong> {(currentYearData.Electricicy / 1000).toExponential(2)} kWh</p>
      </div>
